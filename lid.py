...
def lid_mle(data, reference, k=20, get_idx=False, compute_mode='donot_use_mm_for_euclid_dist'):
    b = data.shape[0]
    k = min(k, b-2)
    data = torch.flatten(data, start_dim=1)
    reference = torch.flatten(reference, start_dim=1)
...
def lid_mom_est(data, reference, k, get_idx=False, compute_mode='donot_use_mm_for_euclid_dist'):
    b = data.shape[0]
    k = min(k, b-2)
    data = torch.flatten(data, start_dim=1)
    reference = torch.flatten(reference, start_dim=1)

...
class BEATs(nn.Module):
    ...
    def extract_features(self, fbank: torch.Tensor, mask_t_prob=0.0, mask_f_prob=0.0):
    ...
    def forward(self, x, mask_t_prob=0.0, mask_f_prob=0.0):
        x = self.extract_features(x, mask_t_prob, mask_f_prob)
...
def beats_classifier(ckpt="checkpoints/BEATs_iter3.pt", **kwargs):
    ckpt = torch.load(ckpt, map_location="cpu", weights_only=False)
    cfg = BEATsConfig(ckpt["cfg"])
    model = BEATs(cfg, **kwargs)
...
def _beats_encode_layers(self, fbank):
    """Run BEATs encoder and collect per-layer outputs. Returns list of 12 [B, L, D] tensors."""
    features = self.patch_embedding(fbank)
    features = features.reshape(features.shape[0], features.shape[1], -1)
    features = features.transpose(1, 2)
    features = self.layer_norm(features)
    if self.post_extract_proj is not None:
        features = self.post_extract_proj(features)
    x = self.dropout_input(features)
    # Manually run encoder layers to collect outputs
    x_conv = self.encoder.pos_conv(x.transpose(1, 2))
    x_conv = x_conv.transpose(1, 2)
    x = x + x_conv
    ...
    for layer in self.encoder.layers:
        x, z, pos_bias = layer(x, self_attn_padding_mask=None, need_weights=False, pos_bias=pos_bias)
        layer_results.append(x.transpose(0, 1))  # T x B x C -> B x T x C
    return layer_results


class BEATsLayerProbe(BEATs):
    """Layer-wise linear probe for BEATs."""
    def __init__(self, cfg, probe_layer=11, pooling='avg', **kwargs):
    ...
    def forward(self, x, mask_t_prob=0.0, mask_f_prob=0.0):
        layer_results = _beats_encode_layers(self, x)
...
class BEATsWeightedSumProbe(BEATs):
    """SUPERB-style weighted-sum linear probe for BEATs."""
    def __init__(self, cfg, depth=12, pooling='avg', **kwargs):
        super().__init__(cfg, **kwargs)
        self.pooling = pooling
...
class BEATsAttentiveProbe(BEATs):
    """Attentive probing for BEATs."""
    def __init__(self, cfg, depth=12, pooling='avg', num_attn_heads=1, embed_dim=768, **kwargs):
        super().__init__(cfg, **kwargs)
        self.pooling = pooling
...
def _load_beats_probe(probe_cls, ckpt="checkpoints/BEATs_iter3.pt", **kwargs):
    ckpt_data = torch.load(ckpt, map_location="cpu", weights_only=False)
    cfg = BEATsConfig(ckpt_data["cfg"])
    model = probe_cls(cfg, **kwargs)
    model.load_state_dict(ckpt_data["model"], strict=False)
    return model

def beats_layer_probe(ckpt="checkpoints/BEATs_iter3.pt", **kwargs):
    return _load_beats_probe(BEATsLayerProbe, ckpt, **kwargs)

def beats_weighted_sum_probe(ckpt="checkpoints/BEATs_iter3.pt", **kwargs):
    return _load_beats_probe(BEATsWeightedSumProbe, ckpt, **kwargs)

def beats_attentive_probe(ckpt="checkpoints/BEATs_iter3.pt", **kwargs):
    return _load_beats_probe(BEATsAttentiveProbe, ckpt, **kwargs)

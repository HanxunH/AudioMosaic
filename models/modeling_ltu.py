...
from modeling_ltu.transformers.activations import ACT2FN
from modeling_ltu.transformers.modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast, SequenceClassifierOutputWithPast
from modeling_ltu.transformers.modeling_utils import PreTrainedModel
from modeling_ltu.transformers.utils import add_start_docstrings, add_start_docstrings_to_model_forward, logging, replace_return_docstrings
from modeling_ltu.transformers.models.llama.configuration_llama import LlamaConfig
from .audiomosaic import AudioMosaicTransformer
...
class LTULlamaModel(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        if audio_input != None: # [B, t, f]
            ...
            if audio_input.shape[1] == 513:
                audio_input = audio_input[:, 1:, :]  # [B, 512, 768] remove cls token
...
class LTULlamaForCausalLM(LlamaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LTULlamaModel(config)
...
class AudioMosaicLlamaModel(LTULlamaModel):
    def __init__(self, config: LlamaConfig):
        super().__init__(config)
        self.audio_encoder = AudioMosaicTransformer(
            spec_size=[1024, 128],
            patch_size=[16, 16],
            depth=12,
            embed_dim=768,
...
class AudioMosaicLlamaForCausalLM(LTULlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.model = AudioMosaicLlamaModel(config)